...
<main>
    <table>
       ...
       <c:set var="uitemsToView" value="${sessionScope.itemsToView}"/>
        <c:forEach items = "${itemsToView}"  var= "itemsToView">
            <tr>
                <td><c:out value = "${itemsToView.itemCode}"/></td>
                <td><c:out value = "${itemsToView.itemName}"/></td><!-- returns selected item name-->
                <td><img src = "<c:out value = "${itemsToView.imageUrl}"/>" alt="partImage"></td><!-- i could not add the img source from the items rating, but since is have the selected image object it should work only for one part -->


                <td>&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;<c:out value="${itemsToView.rating}" escapeXml="false"/></td> 
                <td>
                    <!-- this was categories --> 
                    <form action="admin" method="post">
                        <input type = "hidden" value = "<c:out value = "${itemsToView.itemCode}"/>" name = "itemCodeUpdate"> <!-- TO be fixed-->
        ...
        <a href="additem.jsp" class="r">Add an Item</a><br><br>
    <%--
        <%
            /**
